...
gulp.task('uglify_check', function (cb) {
  pump([gulp.src(js_main_path), babel(), uglify()], cb);
});
...
gulp.task('css_main', function () {
  return gulp
    ...
    .pipe(
      autoprefixer({
        browsers: ['last 2 versions', 'Firefox > 10', 'ie >= 8'],
      })
    )
...
gulp.task('rev', function () {
  return gulp.src(['dist/js/**', 'dist/css/**']).pipe(rev()).pipe(rev.manifest()).pipe(gulp.dest('./'));
});
// 添加版本号（路径替换）
